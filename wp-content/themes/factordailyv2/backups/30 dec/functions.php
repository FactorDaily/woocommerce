...
function factordailyv2_setup() {
	...
	add_image_size( 'lead2', 744, 419, array('center','center') ); // Image for the lead story 2
	add_image_size( 'lead3', 540, 304, array('center','center') ); // Lead story 3
	add_image_size( 'lead4_vertical', 412, 432, array('center','center') ); // Lead story 4 (almost square)
	add_image_size( 'lead5', 233, 132, array('center','center') ); //The small thumbnails for the More stories section
...
//special posts style 
//if(is_single('dabholkar-india-anti-superstition')){
	wp_enqueue_style( 'factordailyv2-tkstyle', get_stylesheet_directory_uri().'/css/tkstyle.css' );
//}
	//swipe slider
	...
	//swipe slider 
	wp_enqueue_script( 'jquery-swipe', get_template_directory_uri() . '/js/slider.js',array('jquery') );
	...
	if ( is_singular() && wp_attachment_is_image() ) {
		wp_enqueue_script( 'factordailyv2-keyboard-image-navigation', get_template_directory_uri() . '/js/keyboard-image-navigation.js', array( 'jquery' ), '20141010' );
	}
...
class fd_news {
	...
	function create_post_type_opinion() {
		...
		$args = array(
			...
			'supports' => array('title','editor','author'),
			'has_archive' => false,
			'rewrite' => array('slug' => 'opinion', 'with_front' => ''),
			'query_var' => true,
			'can_export' => true,
			'taxonomies' => array('category')
		);
...
function category_tag_banner($bannerID) {
		...
		if($banner_enabled == 'false' || $banner_enabled == '' ) {
		  ...
		  ?>
           <div id="container-async" data-paged="12" class="sc-ajax-filter ">
                <ul class="nav-filter">
                    ...
                    <?php endforeach; ?>
					<li><span class="btn btn-default tag_buttons close-button"> <a href="#" data-filter="abc">X</a> </span></li>
...
function fd_sharing_buttons() {
	global $post;
		...
		$content .= '<span><a  href="'.$facebookURL.'" target="_blank" ><i class="fa fa-facebook" aria-hidden="true"></i></a></span>';
		$content .= '<span><a href="'. $twitterURL .'" target="_blank" ><i class="fa fa-twitter" aria-hidden="true"></i></a></span>';
		...
		$content .= '<span><a  href="'.$whatsappURL.'" target="_blank"><i class="fa fa-whatsapp" aria-hidden="true"></i></a></span>';
		...
		$content .= '<span><a href="mailto:"><i class="fa fa-envelope" aria-hidden="true"></i></a></span>';
		$content .= '</div>';
...
$qry = array(
    ...
    ob_start();
		...
		echo '<p>&nbsp;</p><div class="row">';
			...
			echo '<div class="col-md-3 col-sm-3 col-lg-3 col-xs-6 grid-item">';
