<?php
/**
 *WP Post Template: AI Researchers India Bubble chart
 Template Name: AI Researchers India Bubble chart
	...
	<div class="entry-content">
		<div class="container clear">
			<div class="row">
				...
				<div class="col-sm-12 col-md-12 col-lg-12 hidden-xxs">
				<?php  echo "<script src='". get_template_directory_uri() . "/airesearchersindia/d3.js'></script>"; ?>
				
				<?php include_once('airesearchersindia/index.html');?>
				
				<?php  echo "<script src='". get_template_directory_uri() . "/airesearchersindia/jscode.js'></script>"; ?>
				</div>
		...
		<p class="subscribe-link">To get more stories like this on email, <a href="#test-popup" class="open-popup-link"> click here </a> and subscribe to our daily brief.</p>
			...
			<?php
			// get raw date
			$date = get_sub_field('timestamp', false, false);
			
			// make date object
			$date = new DateTime($date);
			if(get_sub_field('correction_comments')) {
			?>
			<small class="credits"><em>Updated at <?php echo $date->format('h:i a') ?> on  <?php echo $date->format('F j, Y'); ?>
				&nbsp;<?php the_sub_field('correction_comments'); ?>
					</em></small> <br><?php
			}
				endwhile;
		...
		<?php if( get_field('disclosure') ): ?>
				<small class="credits"><em><?php the_field('disclosure'); ?></em></small>
		<?php endif; ?>
		</div>
					</div>
